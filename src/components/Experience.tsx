      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24">
        ...
        <div className="space-y-16 md:space-y-24">
          {t.experience.items.map((exp, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative pl-8 border-l border-(--color-border-medium)">
              <div className="absolute left-[-5px] top-0 w-[9px] h-[9px] bg-(--color-fg) rounded-full" />
